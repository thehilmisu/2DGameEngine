...
Knight::Knight() : Character(new Properties("HeroKnight_Idle_0", 200, 200, 100, 55))
{
    m_RigidBody = new RigidBody();
    m_Animation = new Animation();
...
{
    m_Animation->SetProps(m_IdleTextureIDs, 120, SDL_FLIP_NONE);
    m_RigidBody->UnSetForce();

    if(Input::GetInstance()->GetKeyDown(SDL_SCANCODE_RIGHT))
    {
        m_RigidBody->ApplyForceX(KNIGHT_SPEED * FORWARD);
        m_Animation->SetProps(m_RunTextureIDs, 120, SDL_FLIP_NONE);
    }
    if(Input::GetInstance()->GetKeyDown(SDL_SCANCODE_LEFT))
    {
        m_RigidBody->ApplyForceX(KNIGHT_SPEED * BACKWARD);
        m_Animation->SetProps(m_RunTextureIDs, 120, SDL_FLIP_HORIZONTAL);
    }
    if(Input::GetInstance()->GetKeyDown(SDL_SCANCODE_SPACE))
    {
        m_RigidBody->ApplyForceY(KNIGHT_SPEED * UPWARD);
        m_Animation->SetProps(m_JumpTextureIDs, 120, SDL_FLIP_NONE); 
    }
